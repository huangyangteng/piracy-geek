                ...
                >
                    <slot name="header" v-bind:item="item">
                        <div class="d_topMenuBox">
                            <div class="d_menuTitle" v-if="item.name">
                                {{ item.name }}
                            </div>
...
export default {
    ...
    methods: {
        touchStart(event, selectId) {
            ...
            console.log('起始组件位置', OriginObjPosition)
            console.log('起始鼠标位置', OriginMousePosition)
            console.log('鼠标移动的距离', moveLeft, moveTop)
            ...
            function mouseMoveListener(event) {
                moveTop =
                    OriginObjPosition.top +
                    (event.screenY - OriginMousePosition.y)
                moveLeft =
                    OriginObjPosition.left +
                    (event.screenX - OriginMousePosition.x)
            ...
            function cardDetect(moveItemTop, moveItemLeft) {
                ...
                let newHeightNum = Math.round(
                    moveItemTop / that.cardOutsideHeight
                )

                //如果移动卡片至范围外则不会有任何操作，直接返回
                if (
                    newHeightNum >
                        Math.ceil(that.data.length / that.colNum) - 1 ||
                    newHeightNum < 0 ||
                    newWidthNum <= 0 ||
            ...
            function mouseUpListener() {
                ...
                document
                    .querySelector('.d_moveBox')
                    .classList.add('d_transition')
                document.querySelector('.d_moveBox').style.top =
...
*/
                    document
                        .querySelector('.d_moveBox')
                        .classList.remove('d_transition')
                    document
                        .querySelector('.d_moveBox')
                        .classList.remove('d_moveBox')
