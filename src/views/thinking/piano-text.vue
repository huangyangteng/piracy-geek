        ...
        format() {
            ...
            for (let i = 0; i < list.length; i++) {
                ...
                if (item.length > 0) {
                    if (item.length > 150) {
                        // 大于150个字就分成凉拌
                        let half = Math.floor((item.length - 1) / 2)
                        // 从完整的单词开始截取
                        let str2 = item.slice(half)
                        let offset = str2.indexOf(' ')
                        let str1 = item.slice(0, half + offset)
                        str2 = item.slice(half + offset)
                        newList.push(str1)
                        newList.push(space(str1.length))
                        newList.push(str2 + '.')
                        newList.push(space(str2.length))
                    } else {
                        newList.push(item + '.')
                ...
                //  newList.push(list[i] + '.')
                //     newList.push(space(list[i].length))
            }
